...
import { boxLabels, labels, leafCaptions, open, tapBox, tapLeaf } from './page.ts';
...
test.describe('member: a callee no edge reaches', () => {
  ...
  // A closed callee has to be *drawn* to be clickable, and the box exists on the page only because
  // its RETURN node is showing inside it. React Flow would draw an empty box happily, and the reader
  // would have nothing inside it saying which method it is.
  test('a callee of the entry method is drawn as its own name', async ({ page }) => {
    expect(await labels(boxLabels(page))).toEqual(['func1', 'main']);
    expect(await labels(leafCaptions(page))).toEqual(['App', 'app', 'args', 'func1', 'main']);
  });
  ...
  test('a callee of a closed callee is not drawn', async ({ page }) => {
    const boxes = await labels(boxLabels(page));
  ...
  test('clicking a closed box opens it to its own body', async ({ page }) => {
    expect(await labels(leafCaptions(page))).not.toContain('memberA');

    await tapBox(page, 'func1');

    const after = await labels(leafCaptions(page));
    ...
    expect(await labels(boxLabels(page))).toEqual(['func1', 'getMemberX', 'main']);
  });

  test('folding an open box leaves it drawn as a stub, not gone', async ({ page }) => {
    await tapBox(page, 'func1');
    expect(await labels(leafCaptions(page))).toContain('memberA');
  ...
  test('clicking the name of a closed method opens its body', async ({ page }) => {
    ...
    const opened = await labels(leafCaptions(page));
